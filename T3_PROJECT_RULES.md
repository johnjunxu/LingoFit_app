...
### **① 环境变量保护（ENV）**
**[!!!] 严重警告：绝对禁止将任何包含密钥的文件（如 `.env`）提交到 Git 仓库。**

-   **命名规范**: 始终使用 `.env.local` 来存储本地密钥。标准的 `.gitignore` 文件会忽略 `.env.local`，但**不会**忽略 `.env`。错误的命名会导致密钥泄露。
-   **泄露后的补救措施**:
    1.  使用 `git-filter-repo` 等工具从**所有** Git 历史记录中彻底清除泄露的文件。
    2.  立即登录相关服务（如 Supabase, Vercel）**重置 (regenerate)** 所有已泄露的密钥。
    3.  在新部署和本地环境中使用新的密钥。

所有敏感信息必须放在：
- `.env.local`（本地）
- 部署平台的 UI 环境变量设置中（Vercel, Netlify 等）。
